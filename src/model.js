...
let COMPARTMENTS = ['Susceptible', 'Exposed', 'Infected', 'Severely Infected', 'Recovered', 'Dead']

let AGE_COHORTS = ['0-19', '20-59', '60+']

let INFECTION_FATALITY = [.0001, .0032, .0328]
...
function _symmetrize(pop_fracs, contact_data) {
    ...
    f = pop_fracs
    d = contact_data
    c = np.zeros([f.size, f.size])

    f.data.forEach((_, i) => {
        f.data.forEach((_, j) => {
            v = (d.get(i,j)*f.get(i) + d.get(j,i)*f.get(j))/(2*f.get(i))
            c.set(i,j, v)
        })
    })
    return c
...
let CONTACT_MATRICES_0 = {}
Object.keys(WORLD_POP).forEach(k => CONTACT_MATRICES_0[k] = _symmetrize(WORLD_POP[k], CONTACT_DATA))
...
class SEIRModel {
  constructor(
    ...
    SEIRModel.prototype.f = function(t, y) {
        if (isNaN(t)) { throw(new Error("t can't be NaN")) }
        if (y.data) { throw(new Error("y should be an array, not ndarray")) }
        if (y.find(isNaN)) { throw(new Error("y can't contain NaN")) }
        ...
        cohorts_range = Array.from(Array(this.N_cohorts).keys())
        for (a of cohorts_range) {
            let infection_rate = cohorts_range.map(b => (
                   this.beta * contact.get(a,b) * (
                        y.get(b,this.i) + y.get(b,this.m)
                    ) / np.sum(y.pick(b,null))
                    )).reduce((a,b) => a+b)
            dy.set(a,this.s, -y.get(a,this.s) * infection_rate)
            dy.set(a,this.e, (y.get(a,this.s) * infection_rate - this.alpha * y.get(a, this.e)))
            dy.set(a,this.i, (this.alpha * (1-this.kappa[a]) * y.get(a, this.e) - this.gamma * y.get(a, this.i)))
            dy.set(a,this.m, (this.alpha * this.kappa[a] * y.get(a, this.e) - this.delta * y.get(a, this.m)))
            dy.set(a,this.r, this.gamma  * y.get(a, this.i))
            dy.set(a,this.d, this.delta * y.get(a, this.m))
        }
        dy_flat = np.unpack(np.array(dy.data))
        if (dy_flat.find(isNaN)) { throw(new Error("it broke, there's NaN", dy_flat)) }
        return dy_flat
    }

SEIRModel.prototype.solve = function(y0) {
    integrate = new odex.Solver(18)
    epoch_end = this.epoch_end_times.slice(-1)[0]-1
    t = []
    y = []
...
SEIRModel.model_input = function(contact_matrix, day_ranges, selected_npis, total_days, npi_impacts=NPI_IMPACTS) {
    ...
    let _partition = function(day_ranges, total_days) {
        flat_dates = day_ranges.flat().concat(0, total_days).sort((a,b) => a-b)
    ...
    for (epoch of epoch_tuples) {
        c_eff = np.clone(contact_matrix)
        for ([day_range, npi] of zip(day_ranges, selected_npis)) {
    ...
    epoch_ends = epoch_tuples.map(e => e[1])
    return [contact_matrices, epoch_ends]
}


_colsum = (a) => np.unpack(a).map(a => {
    return zip(...a).map(i => i.reduce((a,b) => a+b))
})

SEIRModel.prototype.solve_to_dataframe = function(y0, defaulted_output = false) {
    [t, y] = this.solve(y0)
    y = np.array(y, [t.length, this.N_cohorts, this.N_compartments])
    aggregate_nums = _colsum(y)
